...
class DRRTrainer(object):
    def __init__(self,
        ...
        # Initialize device
        self.device = torch.device('cuda:{}'.format(torch.cuda.current_device()) if cuda else "cpu")
    ...
    def load_parameters(self):
        self.state_rep_net.load_state_dict(torch.load(self.config.state_rep_model_trained,map_location=torch.device('cpu')))
        self.actor_net.load_state_dict(torch.load(self.config.actor_model_trained, map_location=torch.device('cpu')))
        self.critic_net.load_state_dict(torch.load(self.config.critic_model_trained, map_location=torch.device('cpu')))
    ...
    def learn(self):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            reward = None
            next_state = None
            while t < self.config.episode_length+1:
                # ---------------------------- start of timestep ---------------------------- #
                ...
                if len(ignored_items) > 0:
                    rec_items = torch.tensor(ignored_items)
                ...
                ignored_items.append(rec_item_idx.clone().detach().to(self.device))
                ...
                if (timesteps > self.config.learning_start) and (len(replay_buffer) >= self.config.batch_size) and (timesteps % self.config.learning_freq == 0):
                    critic_loss, actor_loss, critic_params_norm = self.training_step(timesteps, replay_buffer, True)
            ...
            if epoch % self.config.saving_freq == 0:
                ...
                export_plot(epi_avg_rewards, 'Average Episodic Reward (Training)', self.config.train_mean_reward_plot_dir)
    
        print('Training Finished')
    
        # Save final model parameters
        torch.save(self.actor_net.state_dict(), self.config.actor_model_dir)
        torch.save(self.critic_net.state_dict(), self.config.critic_model_dir)
        torch.save(self.state_rep_net.state_dict(), self.config.state_rep_model_dir)
    ...
    def online_evaluate(self):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            ignored_items = []
            for i in range(self.config.history_buffer_size):
                emb = candidate_items[pos_user_reviews[i, self.i].long()]
                history_buffer.push(emb.detach().clone())
    ...
    def save_rec(self):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            # Copy item embeddings to candidate item embeddings set
            all_candidate_items = self.item_embeddings.detach().clone().to(self.device)
    
            # Assicuriamoci di includere gli item in pos_user_reviews
            required_indices = set(pos_user_reviews[:, self.i].long().tolist())
            num_additional = 200 - len(required_indices)
            all_indices = set(range(all_candidate_items.shape[0]))
            additional_indices = random.sample(all_indices - required_indices, max(0, num_additional))
    
            selected_indices = list(required_indices) + list(additional_indices)
            candidate_items = all_candidate_items[selected_indices]
    
            # Crea una mappatura tra i vecchi indici e i nuovi
            old_to_new_index = {old_idx: new_idx for new_idx, old_idx in enumerate(selected_indices)}
            ...
            usr = {
                'usr': user_emb.numpy().tolist(),
                'items': candidate_items.numpy().tolist(),
                'rec_items': []
            }
            ...
            # Fill history buffer with positive user item embeddings
            for i in range(self.config.history_buffer_size):
                old_idx = pos_user_reviews[i, self.i].long().item()
                new_idx = old_to_new_index[old_idx]
                emb = candidate_items[new_idx]
                history_buffer.push(emb.detach().clone())
            # Starting item index
            t = 0
            ...
            while t < self.config.episode_length:
                ...
                original_idx = selected_indices[rec_item_idx.item()]
                usr['rec_items'].append(rec_item_emb.numpy().tolist())
        
                if original_idx in user_reviews[:, self.i]:
                    # Reward from rating in dataset if item rated by user
                    reward = user_reviews[user_reviews[:, self.i] == original_idx, self.r][0]
                else:
                    # Item not rated by user, reward from PMF
                    with torch.no_grad():
                        reward = self.reward_function(torch.tensor([e]).to(self.device), torch.tensor([original_idx]).to(self.device))
        ...
        with open(self.config.output_path + 'users_data.json', 'w') as f:
            json.dump(us, f)
